...
@Component({
  selector: 'app-site-treeview-all-sites-v2',
  templateUrl: './site-treeview-all-sites-v2.component.html',
  styleUrls: ['./site-treeview-all-sites-v2.component.scss']
})
export class SiteTreeviewAllSitesV2Component extends TreeviewComponent implements OnInit {
  disableSave = false;
  ...
  @Output() showSitesTree = new EventEmitter();
  ...
  showTree() {
    if(this.disableBtn) {
      let str = this.translationPipe.getTraslatedValue("userRolesRights","pleaseSelectOrganization");
      const alertEr: AlertModel = { type: AlertType.DANGER, message: str, header: HeaderType.ERROR, isUserExplicitEvent: true };
      this.notification.showNotification(alertEr);
      return;
    }
    this.showSitesTree.emit(false);
  }
