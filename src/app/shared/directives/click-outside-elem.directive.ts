...
@Directive({ selector: '[clickOutsideElem]' })
export class ClickOutsideElemDirective implements OnInit, OnChanges, OnDestroy {
  ...
  @Input() excludeClassList: Array<string> = [];
  @Input() excludeDynamic: boolean = false;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['attachOutsideOnClick'] || changes['exclude'] || changes['emitOnBlur'] || changes['excludeList']) {
      this._init();
    }
  }
  ...
  private _shouldExclude(target): boolean {
    ...
    if (this.excludeDynamic) {
      if (this.excludeClassList.some(className => target.classList.contains(className))) {
        return true;
      }
    }
